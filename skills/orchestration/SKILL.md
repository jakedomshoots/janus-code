...
name: orchestration
description: >-
  Use Janus Code orchestration for structured multi-agent coordination: threaded
  messages, blocking ask/reply flows, task dispatch, worker_done/escalation
  waits, task DAGs, decision gates, coordinator loops, or decomposing work
  across agents. Use `janus-cli` instead for ordinary terminal control,
  lightweight terminal prompts, shell commands, Janus Code worktree management,
  reading or waiting on terminals, and automation of the browser embedded inside
  Janus Code. Use Computer Use for browser windows, webviews, Janus Code app UI, or desktop
  UI outside Janus Code's embedded browser.
---

# Janus Code Inter-Agent Orchestration

Orchestration is Janus Code's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.

Use this skill when coordination state matters. For lightweight terminal prompts or basic worktree/terminal/built-in-browser control, use `janus-cli`.
...
- `janus status --json` should show a running runtime.
- `janus` must be on PATH (`janus` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `janus orchestration` commands are RPC calls to the running Janus Code runtime.
...
```bash
janus orchestration task-list --json
janus terminal list --json
# If inherited context includes a task id:
janus orchestration dispatch-show --task <task_id> --json
```

## Messaging

```bash
janus orchestration send --to <handle|@group> --subject <text> [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
janus orchestration check [--terminal <handle>] [--unread] [--types <type,...>] [--inject] [--wait] [--timeout-ms <n>] [--json]
janus orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
janus orchestration ask --to <handle> --question <text> [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
janus orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; Janus Code auto-resolves it from the current terminal.
- While supervising workers manually, use `check --wait --types worker_done,escalation,decision_gate --timeout-ms <n>` instead of sleep/poll loops. Reply to `decision_gate` messages with `janus orchestration reply --id <msg_id> --body <answer> --json`, then keep waiting.
...
```bash
janus orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
janus orchestration task-list [--status <status>] [--ready] [--json]
janus orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
janus orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
janus orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `janus terminal send --terminal <handle> --text <prompt> --enter --json`.
...
```bash
janus orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
janus orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
janus orchestration gate-list [--task <task_id>] [--status <status>] [--json]
janus orchestration run --spec <text> [--from <handle>] [--poll-interval-ms <n>] [--max-concurrent <n>] [--worktree <selector>] [--json]
janus orchestration run-stop [--json]
```
...
Recovery only: `janus orchestration reset --tasks|--messages|--all --json` clears runtime-global orchestration state. Do not run it during active coordination unless explicitly abandoning that state.
...
```bash
janus worktree create --name <task-name> --agent codex --json
janus terminal list --worktree id:<newWorktreeId> --json
janus terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
janus orchestration dispatch --task <task_id> --to <handle> --inject --json
```

Read the new worktree id from `worktree create`, then use `terminal list` to get the agent terminal handle. Omit `--repo` only when running inside an Janus-managed worktree; otherwise pass `--repo <selector>`. `--agent` reveals the new worktree and launches the selected agent in its first terminal, so do not create a separate startup terminal. Use `--setup run|skip|inherit` when setup behavior matters, and `--no-parent` for independent work.

Use `janus worktree create --prompt ...` or `janus terminal send ...` only for untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.

Other terminal commands coordinators often need:

```bash
janus terminal list [--worktree <selector>] [--json]
janus terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
janus terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
janus terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
janus terminal read --terminal <handle> --json
janus terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `janus terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once, even on failure:
  `janus orchestration send --to <coordinator_handle> --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","filesModified":["path/a"],"reportPath":"<optional>"}' --json`
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `janus orchestration send --to <coordinator_handle> --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
janus worktree create --name login-css-worker --agent claude --json
janus terminal list --worktree id:<newWorktreeId> --json
janus terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
janus orchestration task-create --spec "Fix the login button CSS" --json
janus orchestration dispatch --task <task_id> --to <handle> --inject --json
janus orchestration check --wait --types worker_done,escalation,decision_gate --timeout-ms 900000 --json
```

## Next Action

Coordinator: confirm `janus status --json`, inspect `task-list`/`dispatch-show` if inheriting state, then choose either a manual loop (`task-create` -> worker -> `dispatch --inject` -> `check --wait`) or `orchestration run`.
