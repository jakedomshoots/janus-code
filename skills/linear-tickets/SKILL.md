...
name: linear-tickets
description: >-
  Use Janus Code's Linear CLI to read linked ticket context, post completion updates,
  ...
  parented follow-up creation for Linear-linked Janus Code tasks without treating
...
Use `janus linear` when Linear is the source of task context or ticket updates. On Linux, use `janus` wherever this file says `janus`.
...
If the installed CLI help disagrees with this skill, trust `janus linear --help` for the available command surface and tell the user the skill guidance may be stale.
...
```bash
janus linear issue --current --full --json
```

Use search when the task names a ticket but the current worktree is not linked:

```bash
janus linear search "auth bug" --workspace all --limit 10 --json
janus linear issue ENG-123 --full --json
```
...
janus linear search <query> [--limit <n>] [--workspace <id>|all] [--json]
janus linear team list [--workspace <id>|all] [--json]
janus linear team members --team <key|id> [--workspace <id>] [--json]
janus linear team states --team <key|id> [--workspace <id>] [--json]
janus linear team labels --team <key|id> [--workspace <id>] [--json]
janus linear list [--filter assigned|created|all|completed|open] [--team <key|id>] [--limit <n>] [--workspace <id>|all] [--json]
janus linear status set [<id>] [--current] --to <state> [--workspace <id>] [--json]
janus linear assignee set [<id>] [--current] (--me | --to-id <userId>) [--workspace <id>] [--json]
janus linear assignee clear [<id>] [--current] [--workspace <id>] [--json]
janus linear priority set [<id>] [--current] --to none|low|medium|high|urgent [--workspace <id>] [--json]
janus linear priority clear [<id>] [--current] [--workspace <id>] [--json]
janus linear estimate set [<id>] [--current] --to <number> [--workspace <id>] [--json]
janus linear estimate clear [<id>] [--current] [--workspace <id>] [--json]
janus linear due-date set [<id>] [--current] --to <yyyy-mm-dd> [--workspace <id>] [--json]
janus linear due-date clear [<id>] [--current] [--workspace <id>] [--json]
...
janus linear create --title <title> [--body <text> | --body-file <path|->] [--team <key|id>] [--state <stateId|exact-name>] [--assignee me|<userId>] [--priority none|low|medium|high|urgent] [--estimate <number>] [--due-date <yyyy-mm-dd>] [--label <labelId-or-exact-name>]... [--parent <id> | --parent-current] [--write-id <uuid>] [--workspace <id>] [--json]
```
...
```bash
janus linear team list --workspace all --json
janus linear team states --team <key-or-id> --workspace <workspaceId> --json
janus linear team labels --team <key-or-id> --workspace <workspaceId> --json
janus linear team members --team <key-or-id> --workspace <workspaceId> --json
```

Prefer IDs for automation. Names are accepted only when they exactly and uniquely match in the issue's team.

SSH/remoting note: when running through an SSH-backed remote Janus Code CLI, body files are only supported via stdin (`--body-file -`), not arbitrary remote file paths. Pipe or redirect the body content explicitly.

Use task listing for queue-style work:

```bash
janus linear list --filter assigned --limit 10 --workspace all --json
janus linear list --filter open --team <key-or-id> --workspace <workspaceId> --json
```
...
```bash
janus linear attach --current --url <pr-or-mr-url> --title "PR/MR link" --json
```
...
2. Otherwise try `janus linear status set --current --to "In Review" --json`.
...
```bash
janus linear create --title <title> --parent-current --body-file - --json
```
...
```bash
janus linear issue <id> --workspace <workspaceId> --json
```
...
Confirm `janus status --json` unless already checked this turn, then read the current issue with `janus linear issue --current --full --json`. For completion, attach the PR/MR link, add one completion comment, and move status only when the target state is deterministic and non-regressive.
