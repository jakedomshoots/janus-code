...
name: computer-use
description: >-
  Use Janus Code's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, Janus Code app UI,
  or other desktop UI. Triggers include "computer use", "janus computer", "read
...
Use this skill for desktop UI through `janus computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.

## Preconditions

- Prefer `janus computer ...`; on Linux, use `janus computer ...` if `janus` is unavailable. In this Janus Code worktree, use `./config/scripts/janus-dev computer ...` only when testing the local dev runtime.
...
```bash
janus status --json
janus computer capabilities --json
```

## Core Loop

```bash
janus computer list-apps --json
janus computer get-app-state --app com.spotify.client --json
janus computer click --app com.spotify.client --element-index 42 --json
```
...
```bash
janus computer get-app-state --app com.microsoft.edgemac --json
janus computer get-app-state --app Spotify --json
janus computer get-app-state --app pid:12345 --json
```
...
janus computer capabilities --json
janus computer list-apps --json
janus computer list-windows --app <app> --json
janus computer get-app-state --app <app> --json
janus computer get-app-state --app <app> --restore-window --json
janus computer click --app <app> --element-index <index> --json
janus computer click --app <app> --x 100 --y 100 --json
janus computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
janus computer set-value --app <app> --element-index <index> --value "text" --json
janus computer type-text --app <app> --text "text" --json
janus computer press-key --app <app> --key Return --json
janus computer hotkey --app <app> --key CmdOrCtrl+A --json
janus computer paste-text --app <app> --text "text" --json
janus computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
janus computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
janus computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | janus computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```bash
janus computer get-app-state --app com.microsoft.edgemac --restore-window --json
janus computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
janus computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `janus computer ... --app Gmail` unchanged because `janus computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `janus computer permissions --id screenshots --json`.
- `accessibility_error`: run `janus computer capabilities --json`; if the message names Accessibility permission, run `janus computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `janus computer permissions --json`, or `janus computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm Janus Code status unless already checked, then run `janus computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `janus computer get-app-state --app <app> --json`.
