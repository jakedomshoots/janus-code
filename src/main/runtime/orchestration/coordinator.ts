...
export class Coordinator {
  ...
  private async dispatchTask(task: TaskRow, targetHandle: string): Promise<void> {
    ...
    // Why: agents dispatched by the coordinator must use janus-dev in dev mode
    ...
    const preamble = buildDispatchPreamble({
      ...
      coordinatorHandle: this.opts.coordinatorHandle,
      devMode:
        process.env.ORCA_USER_DATA_PATH?.includes('janus-dev') ||
        process.env.ORCA_USER_DATA_PATH?.includes('orca-dev'),
