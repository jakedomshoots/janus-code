...
// Why: the dispatch preamble teaches agents about Janus Code's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use janus-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Janus Code instance (Section 6.4).
  const cli = params.devMode ? 'janus-dev' : 'janus'

  const header = `You are working inside Janus Code, a multi-agent IDE. You are a dispatched worker.
