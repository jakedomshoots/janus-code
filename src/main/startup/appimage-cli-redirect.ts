...
export function maybeRedirectAppImageCliLaunch(options: RedirectOptions = {}): RedirectResult {
  ...
  if (!existsSync(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Janus CLI entrypoint at ${cliEntryPath}\n`)
...
function buildElectronRunAsNodeEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const childEnv = { ...env }
  childEnv.JANUS_NODE_OPTIONS = env.NODE_OPTIONS ?? ''
  childEnv.JANUS_NODE_REPL_EXTERNAL_MODULE = env.NODE_REPL_EXTERNAL_MODULE ?? ''
