...
function devWrapperTestEnv(extra: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith('ORCA_DEV_') || key.startsWith('JANUS_DEV_')) {
      delete env[key]
    }
  }
...
describe('run-electron-vite-dev', () => {
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    expect(envSnapshot.dockTitle).toBe('Janus Code: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData janus and janus-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'janus-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        cwd: resolve('.'),
        env: devWrapperTestEnv({
          JANUS_DEV_USER_DATA_PATH: userDataPath,
          ORCA_ELECTRON_VITE_CLI: fakeCliPath,
          ORCA_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          ORCA_SKIP_DEV_WEB_PREPARE: '1',
          ORCA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'janus-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'janus'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('JANUS_USER_DATA_PATH')
