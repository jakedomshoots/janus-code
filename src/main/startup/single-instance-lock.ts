...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Janus Code instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Janus Code process is running, this may be an Electron/macOS single-instance lock failure.'
...
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] ORCA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Janus Code instance running for the same profile.'

/**
 * Why: Janus Code writes two canonical discovery files into `<userData>/`:
 ...
 * instance quits, metadata points at a dead pid and `janus status` reports
 ...
 * way dev (`janus-dev` userData) and packaged (`Janus Code` userData) runs lock in
