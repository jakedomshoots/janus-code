...
export function encodePairingOffer(offer: PairingOffer): string {
  ...
  return `janus://pair?code=${base64url}`
}

export function decodePairingOffer(url: string): PairingOffer {
  const code = extractPairingCodeFromUrl(url)
  if (!code) {
    throw new Error('Invalid pairing URL: must start with janus://pair and include a pairing code')
  }
...
function extractPairingCodeFromUrl(url: string): string | null {
  ...
  if ((parsed.protocol !== 'janus:' && parsed.protocol !== 'orca:') || parsed.hostname !== 'pair') {
    return null
  }
...
// Why: accept either a `janus://pair?...` URL, legacy `orca://pair?...` URL, or the bare base64
...
export function parsePairingCode(input: string): PairingOffer | null {
  ...
  try {
    ...
    if (lower.startsWith('janus://') || lower.startsWith('orca://')) {
