  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Janus Code runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Janus Code runtime metadata at ${metadataPath}. Start Janus Code first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  if (process.env.JANUS_USER_DATA_PATH) {
    return process.env.JANUS_USER_DATA_PATH
  }
  if (process.env.ORCA_USER_DATA_PATH) {
    return process.env.ORCA_USER_DATA_PATH
  }
  if (platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'janus-code')
  }
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Janus Code runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'janus-code')
  }
  ...
  return join(process.env.XDG_CONFIG_HOME || join(homeDir, '.config'), 'janus-code')
}
