...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Janus Code may show a system prompt to register the Janus CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Janus Code needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Janus CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Janus CLI in PATH.'
          )
    )
    return null
  }
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Janus CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the Janus CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Janus CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (!status.pathConfigured) {
    ...
    // Janus Code affordances when that shell can resolve the Janus CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Janus CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Janus CLI directory to PATH before setup.'
          )
      }
    )
  }
