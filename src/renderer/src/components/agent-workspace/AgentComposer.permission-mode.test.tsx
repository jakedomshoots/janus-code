...
describe('AgentComposer permission mode', () => {
  let root: Root
  ...
  it('applies composer permission mode changes through shared agent defaults', async () => {
    await act(async () => {
    ...
    const manualOption = getDocumentButton('Set permission mode: Ask first')
    ...
    expect(mocks.updateSettings).toHaveBeenCalledWith(
      expect.objectContaining({
        agentDefaultArgs: expect.objectContaining({
          codex: '',
          claude: ''
        }),
        agentDefaultEnv: expect.objectContaining({
          goose: {}
        })
      })
    )
