...
describe('connectPanePty', () => {
  ...
  it('resumes from a quit-captured sleeping record when the restored pane leaf changed', async () => {
    const { connectPanePty } = await import('./pty-connection')
    const transport = createMockTransport('fresh-pty')
    transport.connect.mockImplementation(async ({ sessionId }: { sessionId?: string }) => {
      if (sessionId) {
  ...
  it('does not retry remote snapshots for ordinary hidden runtime output', async () => {
    ...
    expect(transport.serializeBuffer).not.toHaveBeenCalled()
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
      expect.stringContaining('Janus Code skipped hidden terminal output'),
      expect.any(Function)
    )
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
