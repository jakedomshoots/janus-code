...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after Not now', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Janus CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-orca.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the Janus CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Janus CLI in CLI-only reminder toast copy', async () => {
    mocks.skillState.installed = true
    await snoozeInitialModal({ linked: true, remote: false, surface: 'modal' })
    await renderPrompt({ linked: true, remote: false, surface: 'modal' })

    expect(toast.warning).toHaveBeenCalledWith(
      'Janus CLI is missing',
      expect.objectContaining({
        description: 'Install the Janus CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Janus CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Janus CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
