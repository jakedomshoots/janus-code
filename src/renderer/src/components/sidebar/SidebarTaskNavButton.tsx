...
export function SidebarTaskNavButton(): React.JSX.Element | null {
  ...
  const canBrowseTaskProviders = repos.some((repo) => isGitRepoKind(repo))
  ...
  const handlePrefetch = React.useCallback(() => {
    if (!canBrowseTaskProviders || resolvedDefaultTaskSource !== 'github') {
      return
    }
    ...
    <ContextMenu>
      <ContextMenuTrigger asChild>
        <button
          type="button"
          onClick={() => {
            // Why: the Tasks page also handles setup/empty states; only the
            // provider shortcuts need a git repo to target.
            openTaskPage()
          }}
          ...
          <span className="hidden items-center gap-1 group-hover:flex group-focus-within:flex">
            {visibleTaskProviders.includes('github') ? (
              <TaskProviderShortcut
                canBrowseTasks={canBrowseTaskProviders}
                label={translate(
