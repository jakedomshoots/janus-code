  ...
  it('fixes Korean round 5 review, integration, and search keyword regressions', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Install the Janus Code skill so agents know to use the Janus CLI.',
        localeValue: '에이전트가 Janus CLI 사용 방법을 알 수 있도록 Janus Code 스킬을 설치합니다.',
        locale: 'ko'
      })
    ).toBe('agents가 Janus CLI를 사용하도록 Janus Code 스킬을 설치하세요.')
    expect(
      repairTranslatedValue({
