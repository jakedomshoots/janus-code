...
# no saved sessions). Your real `janus-dev` profile is left untouched.
#
# Usage:
...
#   JANUS_FRESH_PROFILE_DIR=/some/path ./config/scripts/dev-fresh-profile.sh  # use a fixed dir
set -euo pipefail
...
PROFILE_DIR="${JANUS_FRESH_PROFILE_DIR:-${ORCA_FRESH_PROFILE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/janus-fresh-profile.XXXXXXXX")}}"
mkdir -p "$PROFILE_DIR"

cleanup() {
  if [[ "$KEEP" -eq 0 && -z "${JANUS_FRESH_PROFILE_DIR:-}" && -z "${ORCA_FRESH_PROFILE_DIR:-}" ]]; then
    # Guard rm -rf against accidental empty/unrelated PROFILE_DIR.
    [[ -n "${PROFILE_DIR:-}" && -d "$PROFILE_DIR" && "$PROFILE_DIR" == */janus-fresh-profile* ]] || return 0
    rm -rf "$PROFILE_DIR"
...
JANUS_DEV_USER_DATA_PATH="$PROFILE_DIR" ORCA_DEV_USER_DATA_PATH="$PROFILE_DIR" JANUS_DEV_SHOW_FIRST_RUN_EDUCATION=1 ORCA_DEV_SHOW_FIRST_RUN_EDUCATION=1 pnpm dev
