# Contributing to Janus Code

Thanks for contributing to Janus Code.

## Before You Start

- Keep changes scoped to a clear user-facing improvement, bug fix, or refactor.
- Janus Code targets macOS, Linux, and Windows. Every change must stay compatible with all three platforms unless the code is explicitly guarded by a runtime platform check.
...
- Janus Code must work against local repositories, remote servers, and SSH worktrees. Do not assume a process, file, credential, shell, or network path exists only on the local machine.
- Janus Code supports many CLI agents, integrations, and git providers. Keep generic behavior provider-neutral; guard integration-specific logic behind explicit checks.
...
- **Include your X (Twitter) handle!** We love giving shoutouts to our contributors when we merge features on Janus Code.
...
```bash
brew install --cask jakedom/homebrew-janus-code/janus-code
```

Release candidates use a separate cask token:

```bash
brew install --cask jakedom/homebrew-janus-code/janus-code@rc
```

The two casks conflict because both install `Janus Code.app`. Switch channels with a
normal `brew uninstall --cask` followed by the install for the other channel.
Do not use `--zap` unless you intentionally want to remove local Janus Code state.
