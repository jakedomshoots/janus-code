...
| Plan proposed | No provider-neutral runtime/source-store signal currently fills `AgentWorkspaceSnapshot.plans`. | not currently available | Janus-style plan rendering needs a structured plan source. Terminal text and assistant prose should not be parsed as the primary plan contract. |
