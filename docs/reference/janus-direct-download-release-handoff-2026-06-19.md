...
| --- | --- |
| Push latest code to GitHub | All workflow-assurance commits through `8d7d1fc0d` were pushed to `origin/main` before this handoff. |
| Rebuild the app | `pnpm run build:mac` produced fresh unsigned `.dmg` and `.zip` macOS artifacts in `dist/`. |
| Support no-Apple direct download | `pnpm run verify:direct-download-artifacts -- --release-notes=RELEASE_NOTES.md` passed with unsigned mac artifacts, checksums, and release notes. |
...
These artifacts are intentionally ignored by git. Publish them as GitHub Release assets alongside `dist/SHA256SUMS.txt` and `RELEASE_NOTES.md`.

## No-Apple Distribution Steps

1. Create a GitHub Release for the current commit.
2. Upload both `.dmg` files, both `.zip` files, and `dist/SHA256SUMS.txt`.
3. Use `RELEASE_NOTES.md` as the release body so users know this is an unsigned direct-download build.
