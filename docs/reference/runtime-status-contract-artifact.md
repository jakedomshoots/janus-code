...
The summary also includes `firstSliceContractRequiredFields`, `firstSliceContractInvalidatableFields`, and `firstSliceContractVersionedFields` so sidecar readers can inspect the first slice's minimum payload, validation, and compatibility-version surfaces before loading the full JSON Schema.
