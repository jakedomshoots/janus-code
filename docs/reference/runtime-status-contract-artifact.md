...
The summary also includes `firstSliceContractRequiredFields` and `firstSliceContractInvalidatableFields` so sidecar readers can inspect the first slice's minimum payload and validation surfaces before loading the full JSON Schema.
