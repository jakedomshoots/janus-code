...
The first slice points at `firstSliceContractArtifactPath: src/shared/runtime-status-contract-artifact.json`, which is the checked-in `status.get` contract that Rust or Tauri-side readers must preserve.
