...
The summary artifact is labeled with media type `application/vnd.janus.runtime-porting-domain-summary+json`, `artifactId: janus-runtime-porting-domain-summary`, `schemaVersion: 1`, `sourceRuntime: electron`, and `targetRuntime: tauri` so external readers can reject unknown formats before using the domain plan.
