...
The summary also includes `firstSliceContractRequiredFields`, `firstSliceContractInvalidatableFields`, `firstSliceContractNonNegativeIntegerFields`, and `firstSliceContractVersionedFields` so sidecar readers can inspect the first slice's minimum payload, validation, numeric, and compatibility-version surfaces before loading the full JSON Schema.
