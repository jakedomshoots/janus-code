...
The first slice points at `firstSliceContractArtifactId: janus-runtime-status-contract` and `firstSliceContractArtifactPath: src/shared/runtime-status-contract-artifact.json`, which identify the checked-in `status.get` contract that Rust or Tauri-side readers must preserve.
