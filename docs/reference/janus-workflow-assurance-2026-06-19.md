...
- Installed-app Computer Use runtime sweep: runner implemented for Settings search/back, completed-thread composer state, slash-command picker, browser workbench launch, terminal drawer presence, and Output/Changes/Review right-panel tabs; current execution is blocked by helper Accessibility and Screen Recording permissions, and the smoke script now prints targeted `janus computer permissions` recovery commands when that happens.
