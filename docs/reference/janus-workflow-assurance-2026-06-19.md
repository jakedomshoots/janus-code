...
- Rebuilt unsigned mac direct-download artifacts with `pnpm run build:mac`, regenerated `dist/SHA256SUMS.txt`, and re-ran `pnpm run verify:direct-download-artifacts -- --release-notes=RELEASE_NOTES.md`; refreshed again after the final assurance-gate and smoke-permission updates.
- Installed the refreshed rebuilt arm64 app over `/Applications/Janus Code.app`, launched it, confirmed the installed helper path is `/Applications/Janus Code.app/Contents/Resources/Janus Computer Use.app`, and confirmed runtime metadata before retrying the live smoke.
...
- Installed-app Computer Use runtime sweep: runner implemented for Settings search/back, completed-thread composer state, slash-command picker, browser workbench launch, terminal drawer presence, and Output/Changes/Review right-panel tabs; after reinstalling the refreshed build, `janus computer permissions --json` still reports `accessibility: not-granted` and `screenshots: not-granted` for the installed helper, and the smoke script prints targeted `janus computer permissions` recovery commands when that happens.
...
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| P1       | The runtime sweep is now captured as `pnpm run smoke:janus-workflow`, but local live execution currently reports `accessibility: not-granted` and `screenshots: not-granted` for `/Applications/Janus Code.app/Contents/Resources/Janus Computer Use.app`. | Run the printed `janus computer permissions --id accessibility --json` and `--id screenshots --json` recovery commands, grant the helper permissions once per machine, then run the smoke command before release.  |
